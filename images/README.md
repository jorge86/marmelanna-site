...
**Μην τις επεξεργαστείς μία-μία.** Όλες παράγονται από το
[`../tools/build-photos.py`](../tools/build-photos.py):

```bash
python3 -m pip install --user Pillow      # μία φορά
python3 tools/build-photos.py
```

Το script κάνει τρία πράγματα σε όλες με τον ίδιο τρόπο: κοπή στο ratio της
θέσης, βάθος πεδίου, και **εξισορρόπηση προς τον μέσο όρο του συνόλου**. Το
τρίτο είναι που δίνει τη συνοχή — μετριέται η απόχρωση κάθε λήψης στο κέντρο
του κάδρου και τραβιέται μερικώς προς τον μέσο όρο όλων.

Η εξισορρόπηση αφορά **χρώμα, όχι φωτεινότητα**: ένα κοντινό σε σκούρα
δαμάσκηνα είναι νόμιμα πιο σκοτεινό από έναν ηλιόλουστο οπωρώνα, και
διορθώνοντας και τη φωτεινότητα καίγονταν τα φωτεινά μέρη.

Για νέα φωτογραφία: βάλε την στο `../../Frouta Photos/`, πρόσθεσε γραμμή στο
`PHOTOS` του script (πηγή, κοπή, ratio, πλάτος, όνομα, σημείο εστίασης,
θόλωμα, όριο bytes) και ξανατρέξε το.
